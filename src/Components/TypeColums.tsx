...
const handleDelete = (row: any, table: string) => () => {
  store.dispatch({type: actionTypes.openAccetp, payload: {
    ...
    handleYes: () => {
      switch (table) {
        case 'roles':
          return store.dispatch(Action.auth.DeletedRole((row._id).toString()));
        case 'users':
          return store.dispatch(Action.auth.DeletedUser((row._id).toString()))
        case 'ebooks':
          return store.dispatch(Action.app.deletedManga((row._id).toString()))
        case 'authors':
          return store.dispatch(Action.app.deletedAuthor((row._id).toString()))
        case 'genres':
...
export const columnsAuthors: any = [
  ...
  { width: 200, editable: false, field: "license", headerName: "License", renderCell: (params: any) => (params?.row?.license) === '' ? 'Null' : params?.row?.license?.userName},
...
export const columnsChapters: any = [
  ...
  { width: 150, editable: false, field: "status", headerName: "Trạng thái" },
  { width: 150, editable: false, field: "numLikes", headerName: "Lượt thích", renderCell: (params: any) => params.row?.numLikes?.map((item: any) =>  item?.userName).join(", ")},
...
export const columnsReviews: any = [
  { width: 100, editable: true, hide: true, field: "id", headerName: "Stt" },
  // { width: 120, editable: true, field: "images", headerName: "Avatar", renderCell: (params: any) => params.row.images.wallPaper === '' ? <IconPhoto/> : <Avatar src={'https://nhadat24h.com/uploads/bds/201904/14/926145_083922_4.jpg'} alt='images user'/>},
  { width: 150, editable: false, field: "users", headerName: "Người dùng", renderCell: (params: any) => params.row.users?.userName},
...
export const columnsComments: any = [
  { width: 100, editable: true, hide: true, field: "id", headerName: "Stt" },
  // { width: 120, editable: true, field: "images", headerName: "Avatar", renderCell: (params: any) => params.row.images.wallPaper === '' ? <IconPhoto/> : <Avatar src={'https://nhadat24h.com/uploads/bds/201904/14/926145_083922_4.jpg'} alt='images user'/>},
  { width: 200, editable: false, field: "userId", headerName: "Người dùng", renderCell: (params: any) => params.row.userId?.map((item: any) =>  item.name).join(", ")},
  { width: 200, editable: true, field: "bookId", headerName: "Truyện", renderCell: (params: any) => params.row.bookId?.map((item: any) =>  item.name).join(", ")},
  { width: 250, editable: false, field: "content", headerName: "Nội dung" },
  { width: 150, editable: false, field: "PostId", headerName: "Forum", renderCell: (params: any) => params.row.PostId?.map((item: any) =>  item.name).join(", ")},
  { width: 150, editable: false, field: "reviewId", headerName: "Review", renderCell: (params: any) => params.row.reviewId?.map((item: any) =>  item.name).join(", ")},
  { width: 150, editable: false, field: "chapterId", headerName: "Chapter", renderCell: (params: any) => params.row.chapterId?.map((item: any) =>  item.name).join(", ")},
